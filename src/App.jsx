...
export default function App() {
  ...
  useEffect(() => {
    ...
    return () => { socket.off('authSuccess'); socket.off('balanceUpdate'); socket.off('depositHistory'); socket.off('withdrawalHistory'); socket.off('chatMessage'); socket.off('chatHistory'); };
  }, [ready, authenticated]);
  ...
  const handleSendChat = (e) => {
     ...
     socket.emit('sendChatMessage', { email: user.email, username: user.username, message: chatInput });
     setChatInput("");
  };
      ...
      <div className="app-container">
        ...
        {view === 'lobby' && user && (
            <Lobby 
                ...
                onJoin={(r) => {setActiveRoom(r); setView('game');}} 
                onLogout={handleLogout} 
                onOpenMenu={() => setShowMenu(true)} 
                onOpenHelp={() => setShowTerms(true)} 
        ...
        {/* --- CHAT DRAWER --- */}
        {showChat && user && (
            <div className="modal-overlay" style={{justifyContent: 'flex-end', alignItems: 'stretch'}} onClick={(e) => { if(e.target.className.includes('modal-overlay')) setShowChat(false); }}>
                <div className="menu-drawer slide-in-right" style={{background:'#0f172a'}}>
                    <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', paddingBottom:'10px', borderBottom:'1px solid #1e293b'}}>
                        <h2 style={{margin:0, color:'#fbbf24'}}>GLOBAL CHAT</h2>
                        <button onClick={() => setShowChat(false)} style={{background:'none', border:'none', color:'white', fontSize:'24px'}}>×</button>
                    </div>
                    <div className="chat-area" style={{flex:1, overflowY:'auto', display:'flex', flexDirection:'column', gap:'8px', padding:'10px 0'}}>
                        {chatMessages.map((msg, i) => (
                            <div key={i} style={{fontSize:'12px', background: msg.user === user.username ? '#1e293b' : 'transparent', padding:'5px', borderRadius:'5px'}}>
                                <span style={{fontWeight:'bold', color: msg.user === user.username ? '#fbbf24' : '#3b82f6'}}>{msg.user}:</span> {msg.text}
                            </div>
                        ))}
                        <div ref={chatEndRef}></div>
                    </div>
                    <form onSubmit={handleSendChat} style={{display:'flex', gap:'5px', borderTop:'1px solid #1e293b', paddingTop:'10px'}}>
                        <input className="input-field" value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Type a message..." style={{marginBottom:0}} />
                        <button type="submit" style={{background:'#fbbf24', border:'none', borderRadius:'8px', width:'40px', fontWeight:'bold'}}>➤</button>
                    </form>
                </div>
            </div>
        )}
        ...
        {showMenu && user && (
            <div className="modal-overlay" style={{justifyContent: 'flex-end', alignItems: 'stretch'}} onClick={(e) => { if(e.target.className.includes('modal-overlay')) setShowMenu(false); }}>
                <div className="menu-drawer slide-in-right">
                    ...
                    <div className="menu-user-card" onClick={() => { setShowMenu(false); setShowProfile(true); }}>
                        ...
                        <div className="user-details">
                            ...
                            <div className="menu-uid">ID: {user.id ? user.id.toString() : '---'} ❐</div>
                        </div>
                ...
                <style>{`
                    .menu-drawer { width: 85%; max-width: 320px; background: #0f172a; height: 100%; overflow-y: auto; padding: 20px; display: flex; flex-direction: column; gap: 20px; box-shadow: -5px 0 30px rgba(0,0,0,0.8); z-index: 200; box-sizing: border-box; }
                    .slide-in-right { animation: slideIn 0.3s ease-out; }
                    @keyframes slideIn { from { transform: translateX(100%); } to { transform: translateX(0); } }
                    
                    /* Alignments Fixed with Flex & Shrink */
                    .menu-user-card { display: flex; align-items: center; gap: 15px; background: #1e293b; padding: 15px; border-radius: 12px; cursor: pointer; }
                    .avatar-large { 
                        width: 45px; height: 45px; background: #fbbf24; border-radius: 50%; 
                        display: flex; alignItems: center; justify-content: center; 
                        font-weight: 900; color: black; font-size: 20px; 
                        flex-shrink: 0; line-height: 0; /* FIX Alignment */
                    }
                    ...
                    .menu-uid { font-size: 11px; color: #64748b; margin-top: 4px; }
                    
                    .vip-section { background: #1e293b; padding: 15px; border-radius: 12px; }
                    ...
                    .vip-club { color: #22c55e; cursor: pointer; }
                    .progress-track { height: 6px; background: #0f172a; border-radius: 3px; overflow: hidden; }
                    .progress-fill { height: 100%; background: #22c55e; }
                    .xp-text { font-size: 10px; color: #64748b; margin-top: 6px; text-align: right; }
                    
                    .total-bal-label { color: #64748b; font-size: 12px; }
                    ...
                    .q-icon { font-size: 20px; background: #1e293b; width: 48px; height: 48px; display: flex; align-items: center; justify-content: center; border-radius: 12px; transition: 0.2s; }
                    .q-item:active .q-icon { transform: scale(0.95); background: #334155; }
                    .q-item span { font-size: 10px; color: #94a3b8; font-weight: 600; text-align: center; }
                    
                    .menu-list { display: flex; flex-direction: column; gap: 0; }
                    .list-item { display: flex; justify-content: space-between; align-items: center; padding: 16px 0; border-bottom: 1px solid #1e293b; cursor: pointer; color: #94a3b8; font-size: 13px; font-weight: 600; }
                    .list-item:hover { color: white; }
                    
                    .logout-btn-full { flex: 1; background: #1e293b; border: none; border-radius: 12px; color: #ef4444; font-weight: 800; cursor: pointer; padding: 16px; width: 100%; margin-top: 20px; font-size: 14px; }
                `}</style>
            </div>
        )}
            ...
            {authMode === 'reset' && (
                <>
                    {resetStep === 1 ? (
                          ...
                          <>
                            <input className="input-field" type="text" placeholder="OTP" value={otp} onChange={(e) => setOtp(e.target.value)} />
...
const GlobalStyle = () => (
  <style>{`
    ...
    :root { --bg-dark: #0f172a; --glass: rgba(255, 255, 255, 0.05); }
    body { margin: 0; background: var(--bg-dark); color: white; font-family: 'Outfit', sans-serif; overflow-x: hidden; }
    .app-container { min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; background: #0f172a; }
    ...
    .glass-card { background: #1e293b; border: 1px solid #334155; border-radius: 24px; padding: 30px; width: 90%; max-width: 350px; text-align: center; position: relative; }
    .close-btn { position: absolute; top: 15px; right: 15px; background: none; border: none; color: white; font-size: 20px; cursor: pointer; }
    .input-field { width: 100%; background: #0f172a; border: 1px solid #334155; padding: 14px; border-radius: 12px; color: white; margin-bottom: 12px; box-sizing: border-box; }
    ...
    .lp-login-btn-small { background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); color: white; padding: 8px 20px; border-radius: 20px; cursor: pointer; font-weight: 600; transition: all 0.2s; }
    .lp-hero { padding: 60px 20px; max-width: 800px; display: flex; flex-direction: column; align-items: center; }
    .lp-badge { background: rgba(251, 191, 36, 0.15); color: #fbbf24; font-size: 12px; font-weight: bold; padding: 6px 12px; border-radius: 20px; margin-bottom: 20px; border: 1px solid rgba(251, 191, 36, 0.3); }
