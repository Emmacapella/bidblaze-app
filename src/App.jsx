...
const HowToPlay = ({ onClose }) => (
    <div className="modal-overlay">
      <div className="glass-card modal-content fade-in" style={{textAlign:'left'}}>
        ...
        <p style={{color:'#94a3b8'}}>1. Deposit Crypto<br/>2. Bid ($1.00)<br/>3. Last One Wins!</p>
...
function GameDashboard({ logout, user }) {
  ...
  const handleDeposit = async () => {
    ...
    try {
        ...
        // 1. Create VIEM Client (Stronger than raw provider)
        ...
        // 4. Send to Server
        socket.emit('verifyDeposit', { email: user.email.address, txHash: hash, network: selectedNetwork });
        alert("✅ Sent! Verifying on Blockchain...");
  ...
  useEffect(() => {
    socket.on('depositSuccess', (bal) => { setCredits(bal); setDepositAmount(''); setIsProcessing(false); alert("✅ Verified!"); });
    socket.on('depositError', (msg) => { setIsProcessing(false); alert(`❌ ${msg}`); });
    socket.on('withdrawalSuccess', () => alert("✅ Request Sent"));
    socket.on('withdrawalError', (msg) => alert(`❌ ${msg}`));
...
const GlobalStyle = () => (
  <style>{`
    ...
    .jackpot-core .label { font-size: 12px; color: #64748b; letter-spacing: 2px; }
