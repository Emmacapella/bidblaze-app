...
const WalletVault = ({ onClose, userAddress }) => {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      setStatus("❌ Failed: " + (e.message.slice(0, 30) + "..."));
    }
    setLoading(false);
  };
      ...
      <div className="glass-card modal-content">
        ...
        {activeTab === 'deposit' ? (
          <div className="tab-content fade-in">
            ...
            <p className="status-text">{status || "Tap address to copy"}</p>
          </div>
        ) : (
          <div className="tab-content fade-in">
            <input className="input-field" placeholder="Recipient Address (0x...)" value={withdrawAddr} onChange={e => setWithdrawAddr(e.target.value)} />
            <input className="input-field" type="number" placeholder="Amount (ETH)" value={amount} onChange={e => setAmount(e.target.value)} />
            <button className="action-btn" onClick={handleWithdraw} disabled={loading}>
              {loading ? "Sending..." : "Confirm Withdraw"}
            </button>
...
function GameDashboard({ logout, user }) {
  ...
  if (!gameState) return <div className="loading-screen">Connecting to BidBlaze...</div>;
    ...
    <div className="app-container">
      ...
      <div className="glass-panel history-panel">
        ...
        <div className="history-list">
          {gameState.history.slice(0, 4).map((bid) => (
  ...
  return (
    <div className="landing-container">
      ...
      <div className="landing-content">
        ...
        <p>The Future of Competitive Auctions</p>
        <button className="start-btn" onClick={login}>Enter The Arena</button>
      </div>
    </div>
  );
}
...
const GlobalStyle = () => (
  <style>{`
    ...
    .main-btn.ended { background: #475569; box-shadow: none; cursor: default; }
    ...
    .timer-float.ended { color: var(--red); }
    ...
    .address-box:active { background: #334155; }
    
    .input-field { width: 100%; background: #1e293b; border: 1px solid #334155; padding: 14px; border-radius: 12px; color: white; margin-bottom: 10px; box-sizing: border-box; font-family: inherit; }
