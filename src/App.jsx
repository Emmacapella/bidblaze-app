...
const supabaseAnonKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InpzaG9kZ2puanFpcm1jcWJ6dWptIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjYzMDU1OTgsImV4cCI6MjA4MTg4MTU5OH0.Afq5mBp4oRaNjuumpaxbBIHVI_xMlJW4wkCytvL8p5o';  // ← REPLACE WITH YOUR SUPABASE ANON KEY
...
// --- NEW LANDING PAGE WITH SUPABASE MANUAL OTP AUTH ---
function LandingPage({ onAuthSuccess }) {
  const { login: privyLogin, authenticated: privyAuthenticated, user: privyUser } = usePrivy();
  const { wallets } = useWallets();

  const [authMode, setAuthMode] = useState('home'); // 'home', 'email', 'otp', 'reset', 'resetConfirm'
  ...
  // Auto-login with Privy wallet if already connected
  useEffect(() => {
    if (privyAuthenticated && privyUser) {
      const userEmail = privyUser.email?.address || wallets[0]?.address || "wallet_user";
      const userData = {
        email: userEmail.toLowerCase().trim(),
        username: privyUser.name || "Player"
      };
      localStorage.setItem('bidblaze_user', JSON.stringify(userData));
      onAuthSuccess(userData);
    }
  }, [privyAuthenticated, privyUser, wallets, onAuthSuccess]);
  ...
  const verifyOtp = async () => {
    ...
    if (error) {
      setMessage('Invalid or expired OTP: ' + error.message);
    } else if (data.user) {
      const userData = {
  ...
  const confirmReset = async () => {
    ...
    if (verifyError) {
      setMessage('Invalid OTP: ' + verifyError.message);
      setLoading(false);
      return;
    }
    ...
    <div className="landing-page-wrapper">                                                    
      ...
      <div className="lp-nav">
        ...
        <div>                                                                                       
          {authMode === 'home' && (                                                                   
            <>                                                                                      
              <button className="lp-login-btn-small" onClick={() => setAuthMode('email')} style={{marginRight:'10px'}}>Login</button>                                                         
              <button className="lp-login-btn-small" onClick={privyLogin} style={{background:'#fbbf24', color:'black', border:'none'}}>Connect Wallet</button>                      
            </>                                                                                 
          )}                                                                                      
          {authMode !== 'home' && (                                                                   
            <button className="lp-login-btn-small" onClick={() => { setAuthMode('home'); setMessage(''); setEmail(''); }}>← Back</button>                                                                                  
          )}                                                                                  
        </div>                                                                                
      </div>
      ...
      {authMode === 'home' ? (
        <div className="lp-hero">
            ...
            <div className="lp-action-container">
                <button className="lp-btn-primary" onClick={() => setAuthMode('email')}>
                    LOGIN WITH EMAIL                                                                                
                </button>
                 <button className="lp-btn-secondary" onClick={privyLogin}>                                                                                                        
                    CONNECT WALLET 🚀                                                                          
                 </button>                                                                           
            </div>
        ...
        <div className="glass-card fade-in" style={{marginTop:'50px', maxWidth:'400px'}}>
            <h2 style={{color:'white', marginTop:0}}>
              {authMode === 'email' ? 'Login / Sign Up with Email' :
               authMode === 'otp' ? 'Enter 6-Digit OTP' :
            ...
            {authMode === 'email' && (
              <>
                ...
                <button className="main-btn" onClick={sendOtp} disabled={loading} style={{marginTop:'15px'}}>
                  {loading ? 'SENDING OTP...' : 'SEND OTP'}
                </button>
                <p style={{fontSize:'12px', color:'#64748b', marginTop:'20px', cursor:'pointer'}} onClick={() => setAuthMode('reset')}>
                  Forgot password?
                </p>
              </>
            )}
            ...
            {authMode === 'otp' && (
              <>
                <p style={{color:'#94a3b8', fontSize:'14px', textAlign:'center'}}>Check your email for the 6-digit code</p>
                <input 
                ...
                />
                <button className="main-btn" onClick={verifyOtp} disabled={loading}>
                  {loading ? 'VERIFYING...' : 'VERIFY OTP & CONTINUE'}
                </button>
                <p style={{fontSize:'12px', color:'#64748b', marginTop:'15px', cursor:'pointer'}} onClick={sendOtp}>
                  Resend OTP
                </p>
              </>
            )}
            ...
            {authMode === 'reset' && (
              <>
                <p style={{textAlign:'left', color:'#94a3b8', fontSize:'12px', marginBottom:'5px'}}>Email Address</p>
                <input className="input-field" type="email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} />
                <button className="main-btn" onClick={sendResetOtp} disabled={loading} style={{marginTop:'15px'}}>
            ...
            {authMode === 'resetConfirm' && (
              <>
                <p style={{color:'#94a3b8', fontSize:'14px', textAlign:'center'}}>Enter the OTP sent to your email</p>
                <input 
                  className="input-field" 
                  type="text" 
                  placeholder="123456" 
                  ...
                  onChange={(e) => setOtp(e.target.value.replace(/\D/g, ''))} 
                />
                <p style={{textAlign:'left', color:'#94a3b8', fontSize:'12px', marginBottom:'5px', marginTop:'15px'}}>New Password</p>
                <input className="input-field" type="password" placeholder="••••••••" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} />
                <button className="main-btn" onClick={confirmReset} disabled={loading}>
            ...
            {message && (
              <p style={{fontSize:'13px', color: message.includes('sent') || message.includes('successfully') ? '#22c55e' : '#ef4444', marginTop:'15px', textAlign:'center'}}>
                {message}
              </p>
            )}

            <div style={{marginTop:'30px', textAlign:'center'}}>
              <button className="lp-btn-secondary" onClick={privyLogin} style={{width:'100%', padding:'14px'}}>
                OR CONNECT WALLET
              </button>
            </div>
        </div>
...
export default function App() {
  ...
  return (                                                                                  
    ...
    >                                                                                         
      {customUser ? (                                                                             
        <GameDashboard logout={handleLogout} user={customUser} />
      ) : (                                                                                       
