...
function GameDashboard({ logout, user }) {
  ...
  const [referralStats, setReferralStats] = useState([]); // 🆕 Referrals List
  ...
  useEffect(() => {
    ...
    socket.on('referralStats', (data) => setReferralStats(data)); // 🆕 REFERRAL STATS LISTENER
    socket.on('userData', (u) => {
        // This receives the referral code generated by the backend
          ...
          <div className="glass-card modal-content fade-in" style={{textAlign:'left'}}>
            ...
            <div style={{display:'flex', gap:'15px', marginBottom:'20px'}}>
             ...
             <div style={{marginTop:'20px', borderTop:'1px solid rgba(255,255,255,0.1)', paddingTop:'15px'}}>
                <p style={{color:'#94a3b8', fontSize:'12px', fontWeight:'bold', marginBottom:'10px'}}>YOUR REFERRALS</p>
                <div style={{maxHeight:'150px', overflowY:'auto', background:'rgba(0,0,0,0.2)', borderRadius:'8px', padding:'10px'}}>
                    {referralStats.length === 0 ? (
                        <p style={{fontSize:'11px', color:'#64748b', textAlign:'center', margin:0}}>You haven't referred anyone yet.</p>
                    ) : (
                        <table style={{width:'100%', fontSize:'11px', borderCollapse:'collapse'}}>
                            <thead>
                                <tr style={{color:'#94a3b8', textAlign:'left'}}>
                                    <th style={{paddingBottom:'5px'}}>User</th>
                                    <th style={{paddingBottom:'5px', textAlign:'right'}}>Total Won</th>
                                </tr>
                            </thead>
                            <tbody>
                                {referralStats.map((ref, idx) => (
                                    <tr key={idx} style={{borderBottom:'1px solid rgba(255,255,255,0.05)'}}>
                                        <td style={{padding:'5px 0', color:'white'}}>{ref.email.slice(0,4)}...{ref.email.split('@')[1]}</td>
                                        <td style={{padding:'5px 0', textAlign:'right', color:'#22c55e'}}>${ref.total_won?.toFixed(2)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
      ...
      {showMenu && (
        <div className="modal-overlay" onClick={(e) => { if(e.target.className === 'modal-overlay') setShowMenu(false); }}>
            <div className="slide-menu" style={{
                ...
                overflowY: 'auto' // 🆕 MOBILE FIX: Added scrolling support
            }}>
                ...
                >
                    <div>
                       ...
                       <div style={{color:'#64748b', fontSize:'12px'}}>{userEmail.length > 20 ? userEmail.slice(0,20)+'...' : userEmail}</div>
                    </div>
